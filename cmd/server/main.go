...
func prepare() {
	...
	// принудительная очистка признаков работы Go рутин
	goStatus = serverAPI.GoStatus{
		DriverDB:        false,
		DriverModbusTCP: false,
		QueueModbusTCP:  false,
	}
...
func goDriverDB(ctx context.Context, chStore <-chan []database.StoreType, wg *sync.WaitGroup) {
	...
	wg.Add(1)
	goStatus.DriverDB = true

	defer func() {
		wg.Done()
		goStatus.DriverDB = false
	}()
...
func httpServerLocal() {
	...
	r.Get("/status", goStatus.HandlGoStatus) // Получение статуса сервера
